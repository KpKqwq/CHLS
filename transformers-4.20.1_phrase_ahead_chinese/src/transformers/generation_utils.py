...
from .generation_logits_process import (
    ...
    ForcedPrefixTokenLogitsProcessor,
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        #forced_bos_token_id=101
        #print("I am makiing force_bos id ==101!!!")
        if forced_bos_token_id is not None:
            processors.append(ForcedBOSTokenLogitsProcessor(forced_bos_token_id))
        if prefix_ids is not None:
            processors.append(ForcedPrefixTokenLogitsProcessor(prefix_ids))        
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            if attn_len!=None:
                ...
                #model_kwargs["attention_mask"][:,attn_len:]=0
                #model_kwargs["attention_mask"][:,:attn_len-2]=0
            if prefix_ids==None and suffix_ids==None:
                
                return self.beam_search_ori(
                    input_ids,
                    beam_scorer,
                    logits_processor=logits_processor,
                    stopping_criteria=stopping_criteria,
    ...
    def look_ahead(
        ...
        import copy 
        now_tokens=copy.deepcopy(input_ids)
        now_model_kwargs=copy.deepcopy(model_kwargs)
        
        beam_size,_=now_tokens.size()
        tmp_scores=torch.zeros(beam_size,1)
        while ahead_step<max_aheads:
            model_inputs = self.prepare_inputs_for_generation(now_tokens, **now_model_kwargs)
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            tmp_scores=tmp_scores+torch.index_select(next_token_scores,1,torch.tensor(suffix_id).cuda()).cpu()
            #print(torch.index_select(next_token_scores,1,torch.tensor(suffix_id).cuda()).cpu())

            now_tokens=torch.cat((now_tokens,torch.tensor([[suffix_id]]*beam_size).cuda()),dim=-1)
            now_model_kwargs = self._update_model_kwargs_for_generation(
                outputs, now_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )

            ahead_step+=1
        
        this_tokens_labels=finished_beams.cpu().squeeze(dim=0)
        ...
        if suffix_ids==None:
            ...
            pass
        scores_with_suffix_masks[:,cur_len]=this_tokens_labels.cuda()
        scores_with_suffix[:,cur_len]=scores_with_suffix[:,cur_len]+tmp_scores.squeeze(1).cuda()
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        # init attention / hidden states / scores tuples
        scores = () if (return_dict_in_generate and output_scores) else None
        # 1.the new scores AKLK added to look ahead
        stored_beam_scores=torch.zeros((batch_beam_size,stopping_criteria[0].max_length)).to(input_ids).float()
        stored_beam_scores_with_suffix=torch.zeros((batch_beam_size,stopping_criteria[0].max_length)).to(input_ids).float()
        stored_beam_scores_with_suffix_masks=(stored_beam_scores_with_suffix!=stored_beam_scores_with_suffix)

        finished_beams = (
            torch.zeros(batch_size, num_beams).to(input_ids).eq(-1)
        ) 
        ...
        this_peer_finished = False  # used by synced_gpus 
        ...
        while True:
            ...
            if prefix_ids==None or cur_len!=len(prefix_ids)+2:
                ...
                model_inputs["attention_mask"][:,attn_len:]=0
                outputs = self(
                    **model_inputs,
                    return_dict=True,
                    output_attentions=output_attentions,
            ...
            next_token_logits = outputs.logits[:, -1, :]
            # else:
            #     tmp_vocab_size=21128
            #     next_token_logits = torch.rand(batch_size*num_beams,tmp_vocab_size).to("cuda")
            ...
            # print("forward 时间",second_time-first_time))
            next_token_scores = next_token_scores_processed + beam_scores[:, None].expand_as(next_token_scores)
#            print(next_token_scores)
            ...
            # reshape for beam search
            if model_kwargs.get("complex_len",None)==None:
                complex_len=1
            else:
                complex_len= model_kwargs.get("complex_len")
            #if prefix_ids==None or cur_len<=len(prefix_ids)+2+complex_len-1: 
            if prefix_ids==None or cur_len<=len(prefix_ids)+2+complex_len-1+1: 
                vocab_size = next_token_scores.shape[-1]
                next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
                ...
                next_indices = torch_int_div(next_tokens, vocab_size)
                next_tokens = next_tokens % vocab_size
                #if cur_len==len(prefix_ids)+1+complex_len-1:
                #    print(next_tokens)
                ...
                next_indices = torch.tensor([list(range(num_beams))]).type_as(next_tokens)
                next_tokens=next_tokens.view(batch_size,num_beams)
                next_token_scores=next_token_scores.view(batch_size,num_beams)
                # if cur_len==len(prefix_ids)+2+complex_len-1+1:
                #     print(next_tokens)
                #     print(next_token_scores)
            # stateless
            # if cur_len==2:
            ...
            #2.change:the stored beam scores//added by AKLK
            stored_beam_scores=stored_beam_scores[beam_idx,:]
            stored_beam_scores[:,cur_len]=beam_scores
            stored_beam_scores_with_suffix=stored_beam_scores_with_suffix[beam_idx,:]
            stored_beam_scores_with_suffix_masks=stored_beam_scores_with_suffix_masks[beam_idx,:]


            finished_beams=finished_beams[:,beam_idx]
            finished_beams+=(beam_next_tokens==102) 
            
            #if tokenizer!=None and cur_len==len(prefix_ids)+2 and len(prefix_ids)!=0:
            if tokenizer!=None and cur_len==len(prefix_ids)+2 and len(prefix_ids)!=0:
                target_tokens=list(map(lambda x:tokenizer.convert_ids_to_tokens(x),beam_next_tokens.cpu().tolist()))
                target_masks=torch.tensor(list(map(lambda x:x.startswith("##"),target_tokens)))
                finished_beams+=target_masks.cuda()
            



            #print(stored_beam_scores)
            ...
            all_scores_suffix_index=cur_len-(len(prefix_ids)+2)


            if all_scores_suffix_index<0:
                stored_beam_scores_with_suffix[:,cur_len]=stored_beam_scores[:,cur_len]
            elif max_aheads==None or suffix_ids==None or suffix_ids==[]:
                stored_beam_scores_with_suffix[:,cur_len]=stored_beam_scores[:,cur_len]
            elif all_scores_suffix_index>=0 and all_scores_suffix_index<max_aheads:
                stored_beam_scores_with_suffix[:,cur_len]=stored_beam_scores[:,cur_len]
                #if all_scores_suffix_index==0:
                #stored_beam_scores_with_suffix[:,cur_len]=stored_beam_scores[:,len(prefix_ids)+2]
                #stored_beam_scores_with_suffix[:,cur_len]=stored_beam_scores[:,len(prefix_ids)+2]
                tmp_dict1=self.look_ahead(
                        input_ids=input_ids,
                        suffix_ids=suffix_ids,
                        cur_len=cur_len,
                        model_kwargs=model_kwargs,
                        output_attentions=output_attentions,
                        output_hidden_states=output_hidden_states,
                        scores_with_suffix=stored_beam_scores_with_suffix,
                        scores_with_suffix_masks=stored_beam_scores_with_suffix_masks,
                        finished_beams=finished_beams
                )
                stored_beam_scores_with_suffix=tmp_dict1["scores_with_suffix"]
                stored_beam_scores_with_suffix_masks=tmp_dict1["scores_with_suffix_masks"]
            else:
                stored_beam_scores_with_suffix[:,cur_len]=stored_beam_scores[:,cur_len]
                              

            # if ori_attention_mask!=None and cur_len>len(prefix_ids)+2+complex_len-1:
            #     model_kwargs["attention_mask"]=ori_attention_mask
            if return_dict_in_generate and output_scores:
                beam_indices = tuple((beam_indices[beam_idx[i]] + (beam_idx[i],) for i in range(len(beam_indices))))
        ...
        #print(input_ids)
        #3.change
        return (input_ids,stored_beam_scores_with_suffix,stored_beam_scores_with_suffix_masks)
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if model_kwargs.get("complex_len",None)==None:
                ...
                complex_len= model_kwargs.get("complex_len")
            # if prefix_ids!=None and cur_len>=len(prefix_ids)+2+complex_len:
            #     max_index=[torch.argsort(probs[:,i*vocab_size:(i+1)*vocab_size][0],descending=True)[0].data.tolist() for i in range(num_beams)]

            #     max_score=[probs[:,i*vocab_size:(i+1)*vocab_size][0][max_index[i]] for i in range(num_beams)]
            #     new_probs=torch.zeros(probs.shape).type_as(probs)
            #     for i in range(num_beams):
            #         new_probs[:,i*vocab_size+max_index[i]]=max_score
            #     probs=new_probs
