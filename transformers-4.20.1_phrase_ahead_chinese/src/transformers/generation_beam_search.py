...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        self.group_size=self.num_beams // self.num_beam_groups
        if control_beam!=None and self.control_times!=0:
            self.group_size=self.group_size*(self.group_size-2)
            self.control_times-=1
...
#            tokenizer1 = BertTokenizer.from_pretrained("fnlp/bart-base-chinese")
#            print(tokenizer1.batch_decode(next_tokens[0]))
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() == eos_token_id):
                    ...
                    next_beam_scores[batch_idx, beam_idx] = -float("inf")
