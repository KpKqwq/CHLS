...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if cur_len == 1:
            ...
            scores[:, [i for i in range(num_tokens)]] = -float("inf")
            scores[:, self.bos_token_id] = 0
        return scores

class ForcedPrefixTokenLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] that enforces the specified token as the first generated token.
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        cur_len = input_ids.shape[-1]
        if 1<cur_len <len(self.prefix_ids)+2:
            num_tokens = scores.shape[1]
            #scores[:, [i for i in range(num_tokens) if i != self.prefix_ids[cur_len-2]]] = -float("inf")
            scores[:, [i for i in range(num_tokens)]] = -float("inf")
            scores[:, self.prefix_ids[cur_len-2]] = 0
        return scores
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # set all inf values to min possible value
