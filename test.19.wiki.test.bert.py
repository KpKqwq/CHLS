...
work_dir="results/wiki/sort.number/bert/"
...
eval_dir="dataset/Chinese_LS/wiki_test_processed.txt"
...
for main_word in tqdm(reader.words_candidate):
    ...
    for instance in reader.words_candidate[main_word]:
        for context in reader.words_candidate[main_word][instance]:
            ...
            final_hyp_bert=generate_bert(model_bert, tokenizer_bert, eval_file=None, max_length=128,sentences=[text],mask_words=[complex1],flag_text=flag_text)[0]
            list_final_hypo_bert=filter_list(final_hyp_bert)

            
            if len(list_final_hypo_bert)>2:
                sorted_hyps,sorted_hyps_scores=sort_max50(text,complex1,list_final_hypo_bert,generateion_tool,flag_text)
            else:
                sorted_hyps,sorted_hyps_scores=list_final_hypo_bert,[0.5] if list_final_hypo_bert!=[] else[]
