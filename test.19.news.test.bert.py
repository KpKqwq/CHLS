...
ori_sentences=open("dataset/LS/news.ls.txt").readlines()

import time
begin_time=time.time()

work_dir="results/news/sort.number/bert/"
    ...
    for instance in reader.words_candidate[main_word]:
        for context in reader.words_candidate[main_word][instance]:
            ...
            if len(list_final_hypo_bert)>2:
                sorted_hyps,sorted_hyps_scores=sort_max10(text,complex1,list_final_hypo_bert,generateion_tool,flag_text)
            else:
