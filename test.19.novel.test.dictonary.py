...
from test_20_bert import generate_bert
from test_21_dict import generate_dict,read_dict
...
def generate_tecent(sentences,mask_words,uuid_str="177HS95AN3V7KGDH"):
    results=[]
    for i in range(len(sentences)):
        len_word=len(mask_words[i])
        offset_word=sentences[i].index(mask_words[i])
        url='https://effidit.qq.com/api/'
        form_data={"header":{"fn":"text_polish"},"uuid":uuid_str,"user":"aiw","segment_list":[{"str":sentences[i],"offset":offset_word,"len":len_word}],"k":10,"target_lang":"zh"}
        response = requests.post(url, data=json.dumps(form_data))
        tmp_response_list=[dict1["text"] for dict1 in response.json()["phrase_list"]]
        results.append(tmp_response_list)
    return results
...
work_dir="results/novel/sort.number/dict/"
...
for main_word in tqdm(reader.words_candidate):
    ...
    for instance in reader.words_candidate[main_word]:
        for context in reader.words_candidate[main_word][instance]:
            ...
            final_hyp_dict=generate_dict(dict1,[complex1])[0]
            list_final_hypo_dict=list(filter(set(final_hyp_dict)))

            if len(list_final_hypo_dict)>2:
                sorted_hyps,sorted_hyps_scores=sort_max10(text,complex1,list_final_hypo_dict,generateion_tool,flag_text)
            else:
                sorted_hyps,sorted_hyps_scores=list_final_hypo_dict,[0.5] if list_final_hypo_dict!=[] else[]            
